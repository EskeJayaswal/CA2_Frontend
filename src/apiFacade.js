...
const fetchUserData = ( role ) => { const options = makeOptions("GET",true); //True add's the token
return fetch(URL + `/api/info/${role}`, options).then(handleHttpErrors);
}

const fetchBeerJoke = () => { const options = makeOptions("GET", false); //True add's the token
